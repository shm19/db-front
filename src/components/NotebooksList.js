  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex justify-between items-center mb-6 bg-gray-100 p-4 rounded shadow">
        <h2 className="text-3xl font-bold text-gray-800">Your Notebooks</h2>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full bg-white border border-gray-200 rounded-lg shadow-md">
          <thead className="bg-gray-200">
            <tr>
              <th className="px-6 py-3 text-left text-gray-800 text-sm font-semibold uppercase tracking-wide">
                Name
              </th>
              <th className="px-6 py-3 text-left text-gray-800 text-sm font-semibold uppercase tracking-wide">
                Created
          ...
          <tbody>
            {notebooks.map((notebook) => (
              ...
              >
                <td
                  className="px-6 py-4 text-gray-900 text-sm cursor-pointer font-semibold"
                  onClick={() => handleNavigateToNotebook(notebook.id)}
                >
                  {notebook.name}
                </td>
                <td className="px-6 py-4 text-gray-600 text-sm">{notebook.createdAt}</td>
                <td className="px-6 py-4 text-gray-600 text-sm">{notebook.blocks.length}</td>
